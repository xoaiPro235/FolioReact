                                                ...
                                                <div className="py-2">
                                                    {notifications.map((n, idx) => (
                                                        ...
                                                        >
                                                            ...
                                                            {/* Right Content: Actions - Three dots menu */}
                                                            <div className="flex flex-col gap-2 flex-shrink-0 self-center relative group/menu">
                                                                <NotificationMenu
                                                                    onMarkRead={() => !n.read && markNotificationRead(n.id)}
...
const NotificationMenu: React.FC<{ onMarkRead: () => void, onDelete: () => void, isRead: boolean }> = ({ onMarkRead, onDelete, isRead }) => {
        ...
        <div className="relative">
            ...
            {isOpen && (
                <>
                    ...
                    <div className="absolute right-0 top-8 w-44 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-100 dark:border-slate-700 z-[70] overflow-hidden p-1 animate-in fade-in zoom-in-95 duration-150 origin-top-right">
                        {!isRead && (
                            <button
                                onClick={(e) => { e.stopPropagation(); onMarkRead(); setIsOpen(false); }}
                                className="w-full text-left px-3 py-2 text-xs font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-lg flex items-center gap-2 transition-colors"
                            >
