...
import { Plus, Folder, Trash2, ArrowRight, Search, User, Edit2 } from 'lucide-react';
import { useNavigate } from 'react-router-dom';

export const Workspace: React.FC = () => {
  const { projects, currentUser, loadProjectData, createProject, updateProject, deleteProject, workspaceSearchQuery, setWorkspaceSearch, users, loadWorkspaceData } = useStore();
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editProjectId) {
      await updateProject(editProjectId, editName, editDesc);
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-8 h-full overflow-y-auto bg-slate-50 dark:bg-slate-950">
              ...
              <div className="p-6 flex-1 flex flex-col">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    {currentUser && (() => {
                      const role = project.members.find(m => m.userId === currentUser.id)?.role || 'VIEWER';
                      const roleStyles: Record<string, string> = {
                        'OWNER': 'bg-amber-100 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400 border-amber-200 dark:border-amber-500/20',
                        'MEMBER': 'bg-blue-100 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400 border-blue-200 dark:border-blue-500/20',
                        'VIEWER': 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400 border-slate-200 dark:border-slate-700'
                      };
                      return (
                        <span className={`text-[11px] font-bold px-2.5 py-1 rounded-md border shadow-sm uppercase tracking-wider whitespace-nowrap ${roleStyles[role] || roleStyles['VIEWER']}`}>
                          {role}
                        </span>
                      );
                    })()}
                ...
                <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2 truncate">{project.name}</h3>
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl w-full max-w-md p-6 shadow-2xl border border-slate-200 dark:border-slate-800">
            <h2 className="text-xl font-bold mb-4 dark:text-white">Edit Project</h2>
            <form onSubmit={handleUpdate} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-4">
                <button type="button" onClick={() => setIsEditModalOpen(false)} className="px-4 py-2 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg">Cancel</button>
