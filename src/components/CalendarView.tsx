...
export const CalendarView: React.FC = () => {
    ...
    return (
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden h-full flex flex-col shadow-sm">
            ...
            <div className="grid grid-cols-7 grid-rows-5 flex-1 bg-slate-100 dark:bg-slate-800 gap-px border-l border-slate-200 dark:border-slate-800 overflow-y-auto min-h-[600px]">
                {isLoading ? (
                    Array.from({ length: 35 }).map((_, idx) => (
                        <div key={idx} className="bg-white dark:bg-slate-900 p-2 min-h-[120px]">
                            <Skeleton className="h-4 w-6 mb-2" />
                            <Skeleton className="h-3 w-full mb-1" count={2} />
                        </div>
                    ))
                ) : (
                    days.map((day, idx) => (
                        <div key={idx} className={`bg-white dark:bg-slate-900 min-h-[120px] p-2 flex flex-col hover:bg-slate-50 dark:hover:bg-slate-800/80 transition-colors ${!day ? 'bg-slate-50/50 dark:bg-slate-900/50' : ''}`}>
                            {day && (
                                <>
                                    ...
                                    <div className="space-y-1 overflow-y-auto scrollbar-thin max-h-[150px]">
